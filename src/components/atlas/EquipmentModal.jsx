...
import { RARITIES, ARMORS, MATERIALS, statsText, isSellable, sellValueOf } from "@/lib/atlasLoot";
...
function Slot({ label, item, onUnequip, extra, statsLine }) {
  return (<div className="rounded-xl border border-slate-700 bg-slate-800/50 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-400 mb-1">{label}</p>{item ? (<div className="flex items-center justify-between gap-2"><div className="min-w-0"><p className="text-sm font-medium truncate" style={{ color: item.__color }}>{item.__name}</p><p className="text-[11px] text-slate-400">{statsLine ?? displayStats(item.__ref)}{item.__ref?.passive ? ` · ${item.__ref.passive.desc}` : ""}</p>{item.__quality && <p className="text-[10px] text-amber-300 mt-0.5">Calidad: {item.__quality}</p>}{extra && <p className="text-[11px] text-fuchsia-300 mt-0.5">{extra}</p>}</div><button onClick={onUnequip} className="text-[11px] rounded-lg bg-slate-700 hover:bg-slate-600 px-2.5 py-1.5 text-slate-200 whitespace-nowrap">Quitar</button></div>) : <p className="text-sm text-slate-500 italic">Vacío</p>}</div>);
}

function InvRow({ entry, equipped, onEquip, onSell, incompatible }) {
  const rcolor = RARITIES[entry.rarity]?.color;
  const canSell = entry.sellable !== false && isSellable(entry.rarity);
  return (<div className="flex items-center justify-between gap-2 rounded-lg border border-slate-700 bg-slate-800/40 px-3 py-2"><div className="min-w-0"><p className="text-sm font-medium truncate" style={{ color: rcolor }}>{entry.name}</p><p className="text-[11px] text-slate-400">{displayStats(entry.ref)}{entry.ref?.passive ? ` · ${entry.ref.passive.desc}` : ""}</p>{entry.quality && <p className="text-[10px] text-amber-300 mt-0.5">{entry.quality}</p>}{incompatible && <p className="text-[10px] text-rose-400 mt-0.5">Arma incompatible con la clase actual</p>}</div><div className="flex items-center gap-1.5 shrink-0">{equipped ? <span className="text-[11px] text-emerald-300 flex items-center gap-1"><Check className="w-3 h-3" /> Equipado</span> : incompatible ? <span className="text-[11px] text-rose-400 flex items-center gap-1"><Lock className="w-3 h-3" /> Incompatible</span> : <button onClick={onEquip} className="text-[11px] rounded-lg bg-sky-700 hover:bg-sky-600 px-2.5 py-1.5 text-white">Equipar</button>}{canSell ? <button onClick={onSell} className="text-[11px] rounded-lg bg-amber-700 hover:bg-amber-600 px-2.5 py-1.5 text-white">{sellValueOf(entry.rarity)} oro</button> : <span className="text-[11px] text-slate-500 flex items-center gap-1"><Lock className="w-3 h-3" /> No venta</span>}</div></div>);
}
...
function ClassWeaponRow({ id, equipped, onEquip, onSell }) {
  ...
  return (<div className={`rounded-lg border px-3 py-2 ${equipped ? "border-emerald-600/60 bg-emerald-950/20" : "border-slate-700 bg-slate-800/40"}`}><div className="flex items-center justify-between gap-2"><div className="min-w-0"><p className="text-sm font-medium truncate" style={{ color: rcolor }}>{w.name} <span className="text-[10px] text-slate-400 font-normal">· {w.rarity}</span></p><p className="text-[11px] text-slate-400">{classWeaponStats(w)} · {w.desc}</p><p className="text-[11px] text-fuchsia-200 mt-0.5">⚔ {w.ability.name} <span className="text-slate-500">· {w.ability.cost} energía</span>{dg ? <span className="text-sky-400"> · 🎲 {dg.label}</span> : null}</p><p className="text-[10px] text-slate-500 mt-0.5">{w.ability.desc}</p></div><div className="flex flex-col gap-1 shrink-0">{equipped ? <span className="text-[11px] text-emerald-300 flex items-center gap-1"><Check className="w-3 h-3" /> Equipada</span> : <button onClick={() => onEquip(id)} className="text-[11px] rounded-lg bg-sky-700 hover:bg-sky-600 px-2.5 py-1.5 text-white">Equipar</button>}{sellable ? <button onClick={() => onSell(id)} className="text-[11px] rounded-lg bg-amber-700 hover:bg-amber-600 px-2.5 py-1.5 text-white">{w.sell} oro</button> : <span className="text-[11px] text-amber-300 flex items-center gap-1"><Lock className="w-3 h-3" /> Reliquia</span>}</div></div></div>);
}
function entryFor(kind, id) { const ref = kind === "armor" ? ARMORS[id] : ACCESSORIES[id]; if (!ref) return null; return { id, kind, name: ref.name, rarity: ref.rarity, ref, sellable: true }; }

export default function EquipmentModal({ player, onEquipWeapon, onEquipArmor, onEquipAccessory, onEquipClassWeapon, onSellWeapon, onSellArmor, onSellAccessory, onSellMaterial, onSellClassWeapon, onClose }) {
  ...
  const armor = player.armor ? { __name: ARMORS[player.armor]?.name, __color: RARITIES[ARMORS[player.armor]?.rarity]?.color, __ref: ARMORS[player.armor] } : null;
  const acc = player.accessory ? { __name: ACCESSORIES[player.accessory]?.name, __color: RARITIES[ACCESSORIES[player.accessory]?.rarity]?.color, __ref: ACCESSORIES[player.accessory] } : null;
    ...
    <div className="atlas-landscape-dialog fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur px-4 py-6" onClick={onClose}>
      <div className="rounded-2xl bg-slate-900 border border-slate-700 p-5 max-w-lg w-full max-h-[88vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 gap-2 mb-4">
          ...
          <Slot label="Accesorio" item={acc} onUnequip={() => onEquipAccessory(player.accessory)} />
        </div>
        ...
        {wInv.length > 0 && (<div className="mb-3"><p className="text-[11px] uppercase tracking-wider text-slate-400 mb-1.5">Armas de botín</p><div className="space-y-1.5">{wInv.map(e => <InvRow key={e.uid} entry={e} equipped={player.weapon === e.uid} incompatible={!!e.ref && e.ref.offType !== CLASS_OFF_TYPE[player.class]} onEquip={() => onEquipWeapon(e.uid)} onSell={() => onSellWeapon(e.uid)} />)}</div></div>)}
        {aInv.length > 0 && (<div className="mb-3"><p className="text-[11px] uppercase tracking-wider text-slate-400 mb-1.5">Armaduras</p><div className="space-y-1.5">{aInv.map(e => <InvRow key={e.id} entry={e} equipped={player.armor === e.id} onEquip={() => onEquipArmor(e.id)} onSell={() => onSellArmor(e.id)} />)}</div></div>)}
        {accInv.length > 0 && (<div className="mb-3"><p className="text-[11px] uppercase tracking-wider text-slate-400 mb-1.5">Accesorios</p><div className="space-y-1.5">{accInv.map(e => <InvRow key={e.id} entry={e} equipped={player.accessory === e.id} onEquip={() => onEquipAccessory(e.id)} onSell={() => onSellAccessory(e.id)} />)}</div></div>)}
