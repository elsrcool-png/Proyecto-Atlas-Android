    ...
    <div className="atlas-landscape-dialog fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur px-4 py-6" onClick={onClose}>
      <div className="rounded-2xl bg-slate-900 border border-slate-800 max-w-md w-full p-5" onClick={e => e.stopPropagation()}>
        ...
        {player.accessory && ACCESSORIES[player.accessory] && (
          <div className={`rounded-lg border px-3 py-2 mb-3 ${RARITY_COLOR[ACCESSORIES[player.accessory].rarity]} bg-slate-800/60`}>
            <div className="flex items-center justify-between"><span className="text-sm font-medium">{ACCESSORIES[player.accessory].name}</span><button onClick={() => onEquip(player.accessory)} className="text-[10px] text-slate-200 underline">Desequipar</button></div>
            <p className="text-[11px] text-slate-400 mt-0.5">{ACCESSORIES[player.accessory].desc}</p>
          </div>
        )}
        <div className="grid sm:grid-cols-2 gap-2 max-h-[50vh] overflow-y-auto pr-1">
          ...
          {inventory.map(id => { const a = ACCESSORIES[id]; const equipped = player.accessory === id; return (<div key={id} className={`rounded-lg border px-3 py-2 ${equipped ? RARITY_COLOR[a.rarity] + " ring-1 ring-amber-400" : "border-slate-700 bg-slate-800/40"}`}><div className="flex items-center justify-between"><span className="text-sm font-medium text-slate-100">{a.name}</span><span className={`text-[9px] uppercase tracking-wider ${RARITY_COLOR[a.rarity].split(" ")[0]}`}>{a.rarity}</span></div><p className="text-[11px] text-slate-400 mt-0.5">{a.desc}</p><div className="flex gap-1.5 mt-2"><button onClick={() => onEquip(id)} className={`text-[10px] rounded px-2 py-1 font-medium transition ${equipped ? "bg-slate-700 text-slate-300" : "bg-emerald-600 hover:bg-emerald-500 text-white"}`}>{equipped ? "Desequipar" : "Equipar"}</button><button onClick={() => onDiscard(id)} className="text-[10px] rounded px-2 py-1 bg-rose-700 hover:bg-rose-600 text-white transition">Descartar</button></div></div>); })}
        </div>
      </div>
    </div>
  );
