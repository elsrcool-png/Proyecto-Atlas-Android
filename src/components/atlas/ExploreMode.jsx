...
export default function ExploreMode({ game }) {
  ...
  return (
    <div className={`atlas-ui-v3 atlas-explore-root min-h-screen relative flex flex-col ${horizontal ? "atlas-explore-horizontal" : "atlas-explore-vertical"} ${wantsHorizontal ? "atlas-prefers-horizontal" : ""} ${leftHanded ? "atlas-left-handed" : "atlas-right-handed"} atlas-hud-preset-${settings.hudPreset || "balanced"}`} data-hud-density={settings.hudDensity || "adaptive"} style={{ background: "#0a0a0a" }}>
      ...
      {showHub && (<PlayerHub player={player} region={region} missions={game.missions} missionDefs={game.missionDefs} settings={game.settings} onUpdateSettings={game.onUpdateSettings} onUseConsumable={game.onUseConsumable} onEquipWeapon={game.onEquipWeapon} onEquipArmor={game.onEquipArmor} onEquipHelmet={game.onEquipHelmet} onEquipAccessory={game.onEquipAccessory} onSellWeapon={game.onSellWeapon} onSellArmor={game.onSellArmor} onSellHelmet={game.onSellHelmet} onSellAccessory={game.onSellAccessory} onSellMaterial={game.onSellMaterial} onEquipClassWeapon={game.onEquipClassWeapon} onSellClassWeapon={game.onSellClassWeapon} progressionState={game.progressionState} progressionDisplay={game.progressionDisplay} onAcceptGuildContract={game.acceptGuildContract} onClaimGuildContract={game.claimGuildContract} onEquipMasterySkill={game.equipMasterySkill} onEquipMasteryPassive={game.equipMasteryPassive} onUpgradeMasterySkill={game.upgradeMasterySkill} onAcceptSpecialQuest={game.acceptSpecialQuest} onClaimSpecialQuest={game.claimSpecialQuest} onClose={() => setShowHub(false)} />)}
      ...
      {showJournal && (<MissionJournal missions={game.missions} missionDefs={game.missionDefs} region={region} priorityMissionId={game.priorityMissionId} onSetActive={game.setMissionActive} onSetPriority={game.setPriorityMission} specialQuests={game.progressionDisplay?.specialQuests} onAcceptSpecialQuest={game.acceptSpecialQuest} onClaimSpecialQuest={game.claimSpecialQuest} onClose={() => setShowJournal(false)} />)}
