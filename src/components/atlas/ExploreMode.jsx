...
export default function ExploreMode({ game }) {
  ...
  return (
    <div className={`atlas-explore-root min-h-screen relative flex flex-col ${horizontal ? "atlas-explore-horizontal" : "atlas-explore-vertical"} ${wantsHorizontal ? "atlas-prefers-horizontal" : ""} ${leftHanded ? "atlas-left-handed" : "atlas-right-handed"}`} style={{ background: "#0a0a0a" }}>
      <div ref={vpRef} className={`atlas-explore-viewport relative w-full overflow-hidden ${visualScene ? "atlas-green-stable-viewport" : ""}`} style={{ height: separated ? "calc(100dvh - 92px)" : "100dvh" }}>
        ...
        <div className="atlas-top-hud absolute top-9 left-0 right-0 z-20 pointer-events-none flex items-start justify-between p-2 gap-2">
          ...
          <div className="atlas-top-actions flex items-center gap-1 pointer-events-auto">
            <button onClick={() => setPaused(true)} className="flex items-center justify-center rounded-lg bg-slate-950/78 border border-slate-700 w-8 h-8 text-slate-200 hover:bg-slate-800" title="Pausa"><Pause className="w-3.5 h-3.5" /></button>
      ...
      {separated && !inCombat && !paused && (<div className="shrink-0 w-full bg-slate-950/90 border-t border-slate-700 flex items-center justify-between px-4 sm:px-6" style={{ height: 92 }}><div className={leftHanded ? "order-2" : "order-1"}><Joystick key={joystickKey} scale={cScale} onMove={(x, y) => { dir.current.x = x; dir.current.y = y; }} /></div><div className={`flex items-center gap-2 ${leftHanded ? "order-1" : "order-2"}`}><button onClick={toggleRun} className={`rounded-full border-2 text-white shadow-lg active:scale-95 flex items-center justify-center ${runToggle ? "bg-amber-500/95 border-amber-200" : "bg-slate-800/90 border-slate-500"}`} style={{ width: 48 * cScale, height: 48 * cScale }} title={runToggle ? "Desactivar correr" : "Activar correr"}><Footprints className="w-5 h-5" /></button><button onClick={onB} className="rounded-full bg-blue-600/90 border-2 border-blue-300 text-white font-bold shadow-lg active:scale-95 flex items-center justify-center select-none" style={{ width: 56 * cScale, height: 56 * cScale, fontSize: 14 * cScale }}>B</button><button onClick={onA} aria-label={actionReady ? `${proxHint}. Pulsar A` : "Acción no disponible"} className={`rounded-full border-2 text-white font-bold shadow-lg active:scale-95 flex items-center justify-center transition ${actionButtonClass}`} style={{ width: 64 * cScale, height: 64 * cScale, fontSize: 18 * cScale }}>A</button></div></div>)}
